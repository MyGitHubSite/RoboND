...
And then in the process_image function I call the color_thresh2 function each time for identifying the navigable terrain, obstacles, and golden rocks in the camera image.
...
(5) The navigable terrain from the rover image was converted to world coordinates.

(6) The navigable terrain and obstacles in world coordinates were overlayed onto the world map in blue and the obstacles were overlayed onto the worldmap in red.  

A rock finding routine was added to find golden rocks in the rover camera image.
If any rock was found it was mapped onto the worldmap using the steps above.

(7) An output mosaic was created that combined the warped image, original image, navigable terrain, obstacles, and rocks.

Here is the movie I created in training mode:
...
(6) The navigable terrain from the rover image was converted to world coordinates.

(7) the navigable and obstacles in world coordinates were overlayed onto the world map in blue and the obstacles were overlayed onto the worldmap in red.  

Pitch and roll thresholds were used to control when to map navigable terrain and obstacles in order to increase the accuracy of the mapping.

(8) Navigable pixel angles and their distances were calcuated for use in the decision steps.

(9) A rock finding routine was added to find golden rocks in the rover camera image.

If any rock was found the center of the rock was mapped onto the worldmap using the steps above.

The angles and distances to the rocks were calculated for use in the rock collecting routine.
...
Instead of using all navigable angles for calculating the steering angle while moving forward I used only angles beyond a certain distance (15).  This had the effect of focusing on longer angles for steering rather than short angles which for the rover wouldn't be of much use while the rover is moving at speed.  This allowed the rover to navigate the entire map in most cases with the added help of the stop and stuck routine to get out of jams from the dark boulders and while collecting rocks.
...
I also changed the max velocity to 1.8 instead of 2.  Because I only used longer angles there was no need to go too fast.  I added a throttle calculator based on the ratio for narrow navigable angles to all longer navigable angles.  The higher the ratio the faster I allowed the rover to go.  The principle here was that when the pathway was more certain I cranked up the throttle.  When the pathway became more uncertain I slowed down.

I added a rock collecting routine.  When the rover sees the rock it uses the angles to the rock to steer instead of the navigable angles.  Also, I slowed way down as soon as it sees the rock and starts steering towards it.  I modified the stop forward threshold for collecting a rock to 1 so that the rover stopped right in fron of the rock for collection.

I modified the stop routine so that when the rover comes to a halt it will search for a larger number of long angles to get a better restart out of trouble.  Also, I added a calculation to be able to turn in a direction that helps move the rover toward unnavigated terrain when jammed up against a wall.
...
Within the forward, stop, and rock collection routines I added some code to allow for a stuck condition.  The stuck mode will cause the rover to move backwards and turn at an angle that will help create enough navigable angles to move forward successfully.
 ...
 - Stuck in Forward mode: sees enough navigable angles but is jammed up and not moving forward.
 - Stuck in Stop mode: turning not enough to get out of a jam.
 - Stuck while Rock collecting: sees a rock but has an obstacle in the pathway.
...
Simulator settings used were: Screen Resolution=1024x768 and Graphics quality=Good.
